...
Edgelist find_k_edges_full_search_space(const Edgelist& edgelist, const Vec& s, int k, bool undirected, const std::string& model, double epsilon, double c) {
    ...
    for (int iter : tqdm::range(k)) {
        ...
        std::cout << "polarization z_star: " << polarization << std::endl;
        ...
        std::cout << best_edge.first << " -> " << best_edge.second << std::endl;
        std::cout << "polarization with best edge: " << polarization << std::endl;
...
Edgelist find_candidate_edges_bounded_confidence(
    ...
    if (candidate_edges.empty()) {
        std::cout << "No fitting intervals found\n";
